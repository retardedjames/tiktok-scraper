#!/usr/bin/env python3
"""
TikTok Lite scraper for a real Android phone over USB (usbipd → WSL2 → ADB).

Usage:
  python3 mobile_scrape.py <keyword> [--scrolls N] [--batch N] [--out file.json]

Adapted from the GCP/Waydroid scraper one dir up, with these phone-specific
differences:
  - Default ADB (no -s <host:port>) — targets the single USB-attached device.
  - Coords come from phone/coords.py (coords.json override, else auto-scaled).
  - No Waydroid dual-focus handling; standard `dumpsys window` focus check.

Incorporates from the GCP scraper:
...
# Let the user target a specific device if multiple are attached
...
def _foreground_app() -> str:
    """Return the package of the currently focused window (via dumpsys window)."""
...
def main():
    parser = argparse.ArgumentParser(description="TikTok Lite phone scraper (USB)")
